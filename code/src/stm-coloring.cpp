...
struct ThreadTiming {
    ...
    ThreadTiming() : thread_id(-1), start_time(0), end_time(0), nodes_processed(0), retries(0) {}
};

// Function to print thread timing statistics
void printThreadTimings(const std::vector<ThreadTiming>& timings) {
    std::cout << "\n===== Per-Thread Performance Statistics =====\n";
    std::cout << std::setw(10) << "Thread ID" 
              << std::setw(15) << "Time (sec)" 
              << std::setw(15) << "Nodes" 
              << std::setw(15) << "Nodes/sec"
              << std::setw(15) << "Retries"
              << std::setw(15) << "Retry ratio" << std::endl;
    ...
    for (const auto& timing : timings) {
        double thread_time = timing.end_time - timing.start_time;
        double nodes_per_sec = timing.nodes_processed > 0 ? timing.nodes_processed / thread_time : 0.0;
        double retry_ratio = timing.nodes_processed > 0 ? 
                            static_cast<double>(timing.retries) / timing.nodes_processed : 0.0;
        
        std::cout << std::setw(10) << timing.thread_id
                  << std::setw(15) << std::fixed << std::setprecision(4) << thread_time
                  << std::setw(15) << timing.nodes_processed
                  << std::setw(15) << std::fixed << std::setprecision(1) << nodes_per_sec
                  << std::setw(15) << timing.retries
                  << std::setw(15) << std::fixed << std::setprecision(3) << retry_ratio
                  << std::endl;
        
        total_time += thread_time;
        total_nodes += timing.nodes_processed;
        total_retries += timing.retries;
    }
    
    // Calculate averages and totals
    double avg_time = total_time / timings.size();
    double avg_nodes = static_cast<double>(total_nodes) / timings.size();
    double avg_retries = static_cast<double>(total_retries) / timings.size();
    double avg_retry_ratio = total_nodes > 0 ? static_cast<double>(total_retries) / total_nodes : 0.0;
    
    std::cout << "\nAverages across " << timings.size() << " threads:" << std::endl;
    std::cout << "  Time: " << std::fixed << std::setprecision(4) << avg_time << " sec" << std::endl;
    std::cout << "  Nodes: " << avg_nodes << std::endl;
    std::cout << "  Retries: " << avg_retries << " (" << std::fixed << std::setprecision(3) 
              << avg_retry_ratio << " per node)" << std::endl;
    
    std::cout << "\nOverall parallelism efficiency: " 
              << std::fixed << std::setprecision(2) 
              << (total_time / (timings[0].end_time - timings[0].start_time)) 
              << "x" << std::endl;
...
LibITMColorGraph::LibITMColorGraph(int iterations, bool try_bipartite, int num_threads)
    : STMColorGraph(STMType::Libitm, iterations, try_bipartite,num_threads) {}

TL2ColorGraph::TL2ColorGraph(int iterations, bool try_bipartite, int num_threads)
    : STMColorGraph(STMType::TL2, iterations, try_bipartite, num_threads) {}

// Constructor with minimal initialization
STMColorGraph::STMColorGraph(STMType type, int iterations, bool try_bipartite, int num_threads) 
    : stm_type(type),
...
void STMColorGraph::colorGraph(
    ...
    if (num_threads > 0) {
        ...
        active_threads = std::min(num_threads, max_threads);
    } else {
    ...
    std::cout << "Using " << active_threads << " threads " << std::endl;
    ...
    // Create vector to store thread timing data - must be defined outside the if block
    std::vector<ThreadTiming> thread_timings(active_threads);
    ...
    if (remaining > 0) {
        ...
        #pragma omp parallel
        {
            // Initialize thread-local timing data
            ThreadTiming local_timing;
            int thread_id = omp_get_thread_num();
            local_timing.thread_id = thread_id;
            local_timing.start_time = omp_get_wtime();
            local_timing.nodes_processed = 0;
            local_timing.retries = 0;
            ...
            for (size_t batch = 0; batch < num_batches; batch++) {
                ...
                local_timing.nodes_processed += (end_idx - start_idx);
                ...
                for (size_t i = start_idx; i < end_idx; i++) {
                    ...
                    while (!success && retry_count < MAX_RETRIES) {
                        ...
                        if (!success) {
                            retry_count++;
                            local_retries++;
                            local_timing.retries++;  // Track retries in thread timing data
            ...
            // Record end time and save thread stats
            local_timing.end_time = omp_get_wtime();
            thread_timings[thread_id] = local_timing;
        }
        ...
        printThreadTimings(thread_timings);
    }
...
std::unique_ptr<ColorGraph> createSTMColorGraph(const char* stm_type, int iterations, bool try_bipartite, int num_threads) {
    ...
    if (strcmp(stm_type, "tl2") == 0) {
        ...
        return std::make_unique<LibITMColorGraph>(iterations, try_bipartite, num_threads);
    }
}